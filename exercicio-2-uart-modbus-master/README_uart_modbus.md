...
O formato genérico de uma mensagem do Modbus RTU é composto por 4 partes distintas: 
...
No caso do presente trabalho, o final do `payload` (antes do CRC) deve incluir os últimos 4 dígitos da matrícula do aluno para facilitar a identificação nas mensagens exibidas no servidor.

Os **4 bytes** de matrícula são os últimos 4 dígitos da matrícula do aluno. Cada byte contém o valor **inteiro** de um dos dígitos. Ou seja: **não é o valor correspondente na tabela ascii**. 

Exemplo de uso correto:

```C
uint8_t matricula[4] = {1, 2, 3, 4}; // o tipo também pode ser 'char'
```
...
[Código UART](https://gitlab.com/fse_fga/raspberry-pi/uart_raspberry_pi)

A função de CRC-16 a ser utilizada encontra-se na pasta CRC deste repositório.

A configuração para uso da uart é:
```
Baudrate: 115200
Paridade: Sem Paridade
Largura da palavra: 8 bits
```
